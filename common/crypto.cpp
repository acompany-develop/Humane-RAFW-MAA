...
EVP_PKEY *evp_pubkey_from_sgx_ec256(client_sgx_ec256_public_t *sgx_pubkey)
{
    ...
    try
    {
        ...
        if (!OSSL_PARAM_BLD_push_utf8_string(
                param_bld, OSSL_PKEY_PARAM_GROUP_NAME, "prime256v1", 0
            ))
            throw std::exception();
        ...
        if (EVP_PKEY_fromdata_init(pkey_ctx) <= 0) throw std::exception();
...
EVP_PKEY *evp_private_key_from_bytes(const uint8_t buf[32])
{
    ...
    try
    {
        ...
        if (!OSSL_PARAM_BLD_push_BN(param_bld, OSSL_PKEY_PARAM_PRIV_KEY, bn_priv))
            throw std::exception();
        ...
        if (EVP_PKEY_fromdata(
                pkey_ctx, &pkey, EVP_PKEY_KEY_PARAMETERS | OSSL_KEYMGMT_SELECT_PRIVATE_KEY, params
            )
            <= 0)
            throw std::exception();
    }
...
/* 128bit AES-CMACを取得 */
int aes_128bit_cmac(uint8_t key[16], uint8_t *message, size_t message_len, uint8_t mac[16])
{
    ...
    try
    {
        ...
        params[0] =
            OSSL_PARAM_construct_utf8_string(OSSL_MAC_PARAM_CIPHER, (char *)cipher_name.c_str(), 0);
        params[1] = OSSL_PARAM_construct_end();

        if (!EVP_MAC_init(mac_ctx, key, 16, params)) throw std::exception();

        if (!EVP_MAC_update(mac_ctx, message, message_len)) throw std::exception();

        if (!EVP_MAC_final(mac_ctx, NULL, &mac_len, 0)) throw std::exception();

        if (mac_len != 16) throw std::exception();

        if (!EVP_MAC_final(mac_ctx, mac, &mac_len, mac_len)) throw std::exception();
    }
    catch (...)
    {
        return_flag = 1;
...
/* ECDSA署名 */
int ecdsa_sign(uint8_t *message, size_t message_len, EVP_PKEY *pkey, uint8_t r[32], uint8_t s[32])
{
    ...
    try
    {
        ...
        if (!EVP_DigestSignInit(md_ctx, NULL, EVP_sha256(), NULL, pkey)) throw std::exception();

        if (!EVP_DigestSignUpdate(md_ctx, message, message_len)) throw std::exception();

        if (!EVP_DigestSignFinal(md_ctx, NULL, &sig_len)) throw std::exception();
...
/* 電子署名の検証を実行 */
int sha256_verify(
    const uint8_t *msg, size_t msg_len, uint8_t *sig, size_t sig_size, EVP_PKEY *pkey, int *result
)
{
    EVP_MD_CTX *context;
